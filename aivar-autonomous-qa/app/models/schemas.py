...
class TestStep(BaseModel):
    action:Literal["navigate","fill","click","assert_visible","assert_text"]; target:Optional[dict]=None; value:Optional[str]=None
class GeneratedTest(BaseModel):
    id:str; scenario_id:str; name:str; steps:list[TestStep]; business_assertions:list[str]=Field(default_factory=list)
class GenerationResult(BaseModel):
    tests:list[GeneratedTest]
class ExecutionResult(BaseModel):
    test_id:str; status:Literal["PASSED","FAILED","HEALED","ESCALATED"]; duration_ms:int=0; failed_step_index:Optional[int]=None; error:Optional[str]=None; artifacts_dir:Optional[str]=None; healing_action:Optional[str]=None
...
class QualityReport(BaseModel):
    application_url:str; total_planned:int; total_generated:int; total_executed:int; passed:int; healed:int; failed:int; escalated:int; coverage_score:float; coverage_gaps:list[CoverageGap]=Field(default_factory=list); healer_actions:list[str]=Field(default_factory=list); risk:Literal["LOW","MEDIUM","HIGH"]; summary:str
