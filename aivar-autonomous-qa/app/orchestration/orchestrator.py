...
from app.models.schemas import QualityReport,TestScenario
class AIVAROrchestrator:
    ...
    def run(self,url):
        ...
        gen=self.generator.generate(plan); results=[self.executor.run_test(t) for t in gen.tests]
        actions=[f"{r.test_id}: {r.healing_action}" for r in results if r.healing_action]
        passed=sum(r.status=="PASSED" for r in results); healed=sum(r.status=="HEALED" for r in results); failed=sum(r.status=="FAILED" for r in results); esc=sum(r.status=="ESCALATED" for r in results)
        risk="HIGH" if (failed or esc) else ("MEDIUM" if cov.score<settings.COVERAGE_THRESHOLD else "LOW")
        return QualityReport(application_url=url,total_planned=len(plan.scenarios),total_generated=len(gen.tests),total_executed=len(results),passed=passed,healed=healed,failed=failed,escalated=esc,coverage_score=cov.score,coverage_gaps=cov.gaps,healer_actions=actions,risk=risk,summary=f"Planned {len(plan.scenarios)}, generated {len(gen.tests)}, executed {len(results)}, passed {passed}, healed {healed}, failed {failed}, escalated {esc}; coverage {cov.score:.0%}.")
