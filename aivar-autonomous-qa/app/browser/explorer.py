...
class BrowserExplorer:
    def explore(self,url):
        ...
        with sync_playwright() as p:
            ...
            els=[]
            for loc in page.locator("button,input,a,select,textarea").all()[:100]:
                try:
                    tag=loc.evaluate("e=>e.tagName.toLowerCase()")
                    text=(loc.inner_text(timeout=500) if tag in ("button","a") else "").strip()
                    els.append(ElementInfo(tag=tag,text=text[:120],role=loc.get_attribute("role"),name=loc.get_attribute("name"),label=loc.get_attribute("aria-label"),selector_hint=loc.get_attribute("data-testid") or loc.get_attribute("id")))
